...
tj_server =[
     ...
     # 商管-结算系统2.0-oracle生产库
     {
        'server_id': 92,
        'db_id': 141,
     },
     # 商管-结算系统2.0-帆软-oracle生产库
     {
        'server_id': 92,
        'db_id': 140,
     },
...
def gen_series_disk_usage(y_data,p_color):
    ...
    for i in range(len(h)):
        y = '''{ name: 'p_title', type: 'p_line',areaStyle:{normal:{color:'p_color'}},itemStyle:{normal:{color:'p_color',lineStyle:{color:'p_color'}}}, data: y_data}'''
        y = y.replace('p_title',h[i]).replace('y_data',str(d[i])).replace('p_line',config['shape_disk_usage']).replace('p_color',p_color.split(',')[i])
        s.append(y)
    s = str(s).replace('"','')
    return s,"','".join(h)
...
def gen_html(cfg,html_templete):
    ...
    # 磁盘使用率
    x_disk_uage,y_disk_usage   = get_disk_usage_result(config,st_disk_usage.format(cfg['server_id'],cfg['start_date'],cfg['end_date']))
    ...
    # 磁盘读
    x_disk_read, y_disk_read   = get_result(config, st_disk_read.format(cfg['server_id'],cfg['start_date'],cfg['end_date']))
    disk_read_series           = gen_series(config['disk_read_title'], y_disk_read,config['disk_read_color'])
    disk_read_option           = gen_options_io_net(config['disk_read_title'], config['disk_read_title'], x_disk_read, disk_read_series)

    # 磁盘写
    x_disk_write, y_disk_write = get_result(config, st_disk_write.format(cfg['server_id'],cfg['start_date'],cfg['end_date']))
    disk_write_series          = gen_series(config['disk_write_title'], y_disk_write,config['disk_write_color'])
    disk_write_option          = gen_options_io_net(config['disk_write_title'], config['disk_write_title'], x_disk_write, disk_write_series)
    ...
    if  cfg['db_id'] !='':
        ...
        x_db_total, y_db_total = get_result(config,
                                            st_db_total_num.format(str(cfg['server_id']), cfg['start_date'], cfg['end_date']))
        db_total_series = gen_series(config['db_total_title'], y_db_total, config['db_total_color'])
        db_total_option = gen_options_io_net(config['db_total_title'], config['db_total_title'], x_db_total, db_total_series)

        # 数据库活跃连接
        x_db_active, y_db_active = get_result(config,
                                              st_db_active_num.format(cfg['server_id'], cfg['start_date'], cfg['end_date']))
        db_active_series   = gen_series(config['db_active_title'], y_db_active, config['db_active_color'])
        db_active_option   = gen_options_io_net(config['db_active_title'], config['db_active_title'], x_db_active, db_active_series)

        # 数据库QPS
        x_db_qps, y_db_qps = get_result(config,st_db_qps_num.format(cfg['server_id'], cfg['start_date'],cfg['end_date']))
        ...
        # 数据库TPS
        x_db_tps, y_db_tps = get_result(config,st_db_tps_num.format(cfg['server_id'], cfg['start_date'], cfg['end_date']))
        ...
        db_bak_sz_option = gen_options_io_net(config['db_bak_sz_title'], config['db_bak_sz_title'], x_db_bak_sz, db_bak_sz_series)
        ...
        db_bak_tm_option = gen_options_io_net(config['db_bak_tm_title'], config['db_bak_tm_title'], x_db_bak_tm, db_bak_tm_series)

        # 数据库表空间使用率
        x_db_tbs_usage, y_db_tbs_usage = get_disk_usage_result(config,
                                                    st_db_tbs_usage.format(cfg['db_id'], cfg['start_date'], cfg['end_date']))
...
def write_html(cfg):
    if cfg['db_id']!='':
       filename = '{}-运行周报_db_{}.html'.format(get_server(cfg)['server_desc'],cfg['db_id'])
    else:
       filename = '{}-运行周报_server_{}.html'.format(get_server(cfg)['server_desc'], cfg['server_id'])
